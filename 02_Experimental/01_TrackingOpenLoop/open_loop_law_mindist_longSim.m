...
solver = casadi.nlpsol('solver', 'ipopt', prob,options);
 ...
 x_sol_vecArr    = zeros(length(alpha_vec),nx,1000+1);
 u_sol_vecArr    = zeros(length(alpha_vec),nu,1000);
...
for alpha = alpha_vec
    ...
    x_sol_vec = nan(nx,1000+1);
    u_sol_vec = nan(nu,1000);
...
x_sol_vec = reshape(x_sol_vecArr(k,:,:),nx,1000+1);
